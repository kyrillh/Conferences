\scalebox{0.8}{ % Adjust scale if needed
	\begin{tikzpicture}[
		...
		\begin{scope}[shift={(-5, 3)}]
			...
			\node[xshift=4cm, yshift=3cm] (n2) {\bf{Scalable Multilevel Methods}\\[1ex] \footnotesize \textit{TU Dortmund}};
			\node[xshift=-2.25cm, yshift=3cm] (n3) {\bf{FROSch - Domain Decomposition}\\[1ex]\footnotesize \alert{UzK Cologne}\\ \footnotesize TU Freiberg};
			\node[xshift=-5.5cm, yshift=0cm] (n4) {\bf{Parallel In Time Integration}\\[1ex] \footnotesize FzJ Jülich\\ \footnotesize TU Dortmund\\ \footnotesize FAU Erlangen};
			\node[xshift=-2.25cm, yshift=-2.5cm] (n5) {\bf{Supercomputers and}\\\bf{Acceleration Hardware}\\[1ex] \footnotesize FzJ Jülich\\\footnotesize FAU Erlangen};
